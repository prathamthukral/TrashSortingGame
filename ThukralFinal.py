...
whichClicked=-1
#constant width of the image
WIDTH=100

#When game is paused
def pauseScreen(mx,my):
    global done, milliseconds
    #time does not continue to run
    milliseconds+=0
    ...
    while screenIsPaused==True:
        ...
        screen.fill(BLACK)
        #resume button
        pygame.draw.rect(screen,BLUE,(50,50,100,WIDTH))
        #main menu button
        pygame.draw.rect(screen,RED,(SIZE[0]-150,50,100,WIDTH))
        #text prompts
        fontType('Or click Blue Resume button',WHITE,BLACK,100,330,50)
        fontType('Or click Red button for main menu',WHITE,BLACK,100,430,50)
        for event in pygame.event.get():
            #if the person clicks the button
            if event.type==pygame.MOUSEBUTTONDOWN:
                ...
                if 50<=mx<=150 and 50<=my<=150 and button==1:
                    ...
                    break
                elif SIZE[0]-150<=mx<=SIZE[0]-50 and 50<=my<=150 and button==1:
                    screenIsPause=False
                    menu()
                    break
            #if red or blue button is hovered over
            if event.type==pygame.MOUSEMOTION:
                mx,my=event.pos
                if 50<=mx<=150 and 50<=my<=150:
                    pygame.draw.rect(screen,[0,0,200],(50,50,100,WIDTH))
                elif SIZE[0]-150<=mx<=SIZE[0]-50 and 50<=my<=150:
                    pygame.draw.rect(screen,[200,0,0],(SIZE[0]-150,50,100,WIDTH))
            pygame.display.flip()
...
def newPos(index):
    ...
    if coord_list[index][0]-WIDTH<=xCoord<=coord_list[index][0]+WIDTH:
        while True:
            #generates new values till it is valid
            xCoord=random.randint(50,650)
            #if valid - break the loop
            if xCoord<coord_list[index][0]-WIDTH or xCoord>coord_list[index][0]+WIDTH:
                break
...
def pile(heightPile):
    ...
    if heightPile<=6:
        ...
        for x in range(heightPile):
            screen.blit(pygame.image.load("imagesThukral/garbagebag.png"),(0,SIZE[1]-(x*WIDTH)))
    #draw appropriate number of images
        ...
        for x in range(heightPile-6):
            screen.blit(pygame.image.load("imagesThukral/garbagebag.png"),(WIDTH,SIZE[1]-(x*WIDTH)))
    #draw appropriate number of images
    elif heightPile==12:
...
def getObjectByPoint(mx,my):
    ...
    for index in range(DIFFICULTY):
        ...
        if xValue<=mx<=xValue+WIDTH and yValue<=my<=yValue+height_list[index]:
            resIndx = index
            return resIndx
...
#GAME LOOP
def gameLoop(event,gender,milliseconds):
    global droppedCount, recycleCount, garbageCount, DIFFICULTY, seconds, CLOTHING, whichClicked
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if whichClicked==-1:
                    ...
                    if whichClicked != -1:
                        ...
                        if coord_list[whichClicked][0]<=mx<=coord_list[whichClicked][0]+WIDTH and coord_list[whichClicked][1]<=my<=coord_list[whichClicked][1]+height_list[whichClicked] and button==1:
                            #it stops dropping
...
#menu screen
def menu():
    global done, gender,milliseconds
    #reset the timer
    milliseconds=0
    ...
    while not done:
        ...
        for event in pygame.event.get():
            ...
            #menu buttons
            pygame.draw.rect(screen,[250,0,0],(int(SIZE[0]/2)-150,100,300,WIDTH))
            fontType('play',BLACK,RED,300,130,75)
            pygame.draw.rect(screen,[0,250,0],(int(SIZE[0]/2)-150,250,300,WIDTH))
            fontType('settings',BLACK,GREEN,300,270,75)
            pygame.draw.rect(screen,[0,0,250],(int(SIZE[0]/2)-150,400,300,WIDTH))
            fontType('quit',BLACK,BLUE,300,410,75)
            
            #hover
            if event.type==pygame.MOUSEMOTION:
                    mx,my=event.pos
                    #darken the corresponding button: play, settings, or quit
                    if int(SIZE[0]/2)-150<=mx<=int(SIZE[0]/2)+150 and 100<=my<=200:
                            pygame.draw.rect(screen,[200,0,0],(int(SIZE[0]/2)-150,100,300,WIDTH))
                            fontType('play',BLACK,[200,0,0],300,130,75)
                    elif int(SIZE[0]/2)-150<=mx<=int(SIZE[0]/2)+150 and 250<=my<=350: 
                            pygame.draw.rect(screen,[0,200,0],(int(SIZE[0]/2)-150,250,300,WIDTH))
                            fontType('settings',BLACK,[0,200,0],300,270,75)
                    elif int(SIZE[0]/2)-150<=mx<=int(SIZE[0]/2)+150 and 400<=my<=500: 
                            pygame.draw.rect(screen,[0,0,200],(int(SIZE[0]/2)-150,400,300,WIDTH))
        ...
        while True:
            ...
            for event in pygame.event.get(pygame.KEYDOWN):
                ...
                if event.key==pygame.K_f:
                    ...
                    gender='M'
            #text information
            fontType('Gender: '+gender,BLACK,WHITE,50,WIDTH,40)
